# SimpleAccounts UAE Documentation

This folder contains detailed documentation for the SimpleAccounts UAE project.

## Documentation Structure

```
docs/
├── README.md                    # This file
├── DEVELOPMENT.md               # Development environment guide (Coder, DevContainer, Manual)
├── TESTING_STRATEGY.md          # Testing approach for dependency upgrades
├── LESSONS_LEARNED.md           # Important lessons from development
├── coder/                       # Coder workspace documentation
├── devcontainer/                # DevContainer documentation
├── development/                 # Development workflow docs
│   └── PRE_PR_CHECKLIST.md      # Pre-PR validation checklist
└── templates/                   # Document templates
    └── code-review-checklist.md # Code review checklist
```

> **Note:** The Neumorphic design system is documented in [CLAUDE.md](../CLAUDE.md) at the repository root.

## Quick Links

### Getting Started

- [Development Environments](./DEVELOPMENT.md) - Coder, DevContainer, or Manual setup

### Design & Architecture

- [Neumorphic Theme](../CLAUDE.md) - UI design system (in CLAUDE.md)
- [Testing Strategy](./TESTING_STRATEGY.md) - Testing approach

### Coder Workspaces

- [Coder README](../.coder/README.md) - Main Coder documentation

### Development Workflow

- [Contributing](../CONTRIBUTING.md) - How to contribute
- [Pre-PR Checklist](./development/PRE_PR_CHECKLIST.md) - Before creating PRs
- [Code Review](./templates/code-review-checklist.md) - Review checklist

### Reference

- [Lessons Learned](./LESSONS_LEARNED.md) - Avoid common pitfalls
