...
public interface UserJpaRepository extends JpaRepository<User, Integer> {
	...
	//@Query("SELECT new com.simplevat.rest.payroll.UserDto(u.userId,concat(u.firstName,' ',u.lastName)) FROM User u INNER JOIN u.role r WHERE r.roleName = :roleName")
	//public List<UserDto> findApprovedUser(@Param("roleName")String roleName);

	@Query("SELECT new com.simplevat.rest.payroll.UserDto(u.userId,concat(u.firstName,' ',u.lastName)) FROM User u INNER JOIN u.role r WHERE r.roleName IN (:roleName) and u.isActive=true")
