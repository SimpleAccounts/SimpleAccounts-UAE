...
export const getCurrencyDropdown = list => {
  const newList = list
    ? selectCurrencyFactory.renderOptions('currencyName', 'currencyCode', list, strings.Currency)
    : [];
  return newList;
};
...
export const getContactDropDownList = contact_list => {
  ...
  const newList = tmpContact_list
    ? selectOptionsFactory.renderOptions('label', 'value', tmpContact_list, strings.Customer)
    : [];
  return newList;
};
