...
export const postZipCodeError = (value, countryID) => {
  ...
  if (countryID) {
    if (countryID === 229) {
      if (!value) return strings.POBoxNumberIsRequired;
...
export const addressValidation = values => {
  ...
  if (values.countryId == 229) {
    if (values.stateId == '') error.stateId = 'Emirate is required';
  } else {
    if (values.stateId == '') error.stateId = 'State / Provinces is required';
  }
  return error;
};
