...
const InvoiceViewJournalEntries = ({ invoiceId, invoiceType, history, id, invoiceURL, isCNWithoutProduct }) => {
    ...
    const invoice_journal_list = useSelector((state) => getList(state.invoice_view_journal.invoice_journal_list));
    ...
    const columns = useMemo(() => [
            ...
            cell: ({ getValue }) => (
                <ul className="list-none p-0 m-0">
                    {getValue()?.map((item, idx) => (
                        <li key={idx} className="pb-1">{item.transactionCategoryName}</li>
                    ))}
                </ul>
            ),
        },
            ...
            cell: ({ row }) => (
                <ul className="list-none p-0 m-0 text-right">
                    {row.original.journalLineItems?.map((item, idx) => (
                        <li key={idx} className="pb-1"><Currency value={item.debitAmount?.toFixed(6)} /></li>
                    ))}
                </ul>
            ),
        },
        {
            ...
            cell: ({ row }) => (
                <ul className="list-none p-0 m-0 text-right">
                    {row.original.journalLineItems?.map((item, idx) => (
                        <li key={idx} className="pb-1"><Currency value={item.creditAmount?.toFixed(6)} /></li>
                    ))}
                </ul>
            ),
        },
