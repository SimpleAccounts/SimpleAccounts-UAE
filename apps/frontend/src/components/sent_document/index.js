...
class SentInvoice extends React.Component {
  ...
  getInvoiceDataById = () => {
    ...
    const amountInWords = upperCase(
      currencyName + ' ' + toWords.convert(invoiceAmount) + ' ONLY'
    ).replace('POINT', 'AND');
    const vatInWords = vatAmount
      ? upperCase(currencyName + ' ' + toWords.convert(vatAmount) + ' ONLY').replace('POINT', 'AND')
      : '-';
    var type = '';
  ...
  getPostingRequestModel = () => {
    ...
    const postingRequestModel = {
      ...
      vatInWords: vatAmount
        ? upperCase(currencyName + ' ' + toWords.convert(vatAmount) + ' ONLY').replace(
            'POINT',
            'AND'
          )
        : '-',
  ...
  postInvoice = () => {
    ...
    const postingRequestModel = this.getPostingRequestModel();
    this.props.actions
      .postInvoice(postingRequestModel, documentTitle, strings)
      .then(res => {
        if (res.status === 200) {
          if (markAsSent === true) {
            this.props.commonActions.tostifyAlert('success', strings[messageList[2]]);
          } else {
  ...
  unPostInvoice = () => {
    ...
    this.props.actions
      .unPostInvoice(postingRequestModel, documentTitle, strings)
      .then(res => {
        if (res.status === 200) {
          this.props.commonActions.tostifyAlert('success', strings[messageList[0]]);
          setState(false);
