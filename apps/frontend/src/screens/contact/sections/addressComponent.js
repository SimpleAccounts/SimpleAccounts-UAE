...
import { Row, Col, FormGroup, Input, Label } from 'components/migration';
...
class AddressComponent extends React.Component {
  ...
  render() {
    ...
    const { values, touched, errors, country_list, onChange, addressType, disabled } = this.props;
    ...
    return (
      <>
        <Col md="4">
          <FormGroup>
            ...
            <Input
              ...
              onChange={option => {
                if (option.target?.value === '' || this.regExAddress.test(option.target?.value)) {
            ...
            <Select
              ...
              value={
                values?.countryId?.value
                  ? values?.countryId
                  : country_list.find(option => option?.value === values?.countryId)
              }
              ...
              id="countryId"
              name="countryId"
              className={errors?.countryId && touched?.countryId ? 'is-invalid' : ''}
            />
            ...
            <Select
              ...
              value={
                values?.stateId?.value
                  ? values?.stateId
                  : state_list.find(option => option?.value === values?.stateId)
              }
              onChange={option => {
                if (option && option?.value) {
              ...
              id="stateId"
              name="stateId"
              className={errors?.stateId && touched?.stateId ? 'is-invalid' : ''}
            />
        ...
        <Col md="4">
          <FormGroup>
            ...
            <Input
              ...
              onChange={option => {
                if (option.target?.value === '' || this.regExTelephone.test(option.target?.value)) {
                  onChange('telephone', option);
                }
              }}
              value={values?.telephone}
              className={errors?.telephone && touched?.telephone ? 'is-invalid' : ''}
            />
        ...
        <Col md="4">
          <FormGroup>
            <Label htmlFor="fax">{strings.Fax}</Label>
            <Input
              type="text"
              maxLength="15"
              id="fax"
            ...
            />
            {errors?.fax && touched?.fax && <div className="invalid-feedback">{errors?.fax}</div>}
          </FormGroup>
        </Col>
      </>
    );
