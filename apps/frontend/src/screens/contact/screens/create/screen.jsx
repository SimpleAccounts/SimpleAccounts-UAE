...
const createContactSchema = z.object({
	...
	currencyCode: z.object({
		value: z.number(),
		label: z.string(),
	}).nullable().refine((val) => val !== null, 'Currency is required'),
	contactType: z.object({
		value: z.number(),
		label: z.string(),
	}).nullable().refine((val) => val !== null, 'Contact type is required'),
	taxTreatmentId: z.object({
		value: z.number(),
		label: z.string(),
	}).nullable().refine((val) => val !== null, 'Tax Treatment is required'),
...
const CreateContact = ({
	...
	const initializeData = () => {
		...
		if (companyDetails) {
			...
			const currencyOption = currency_list_dropdown?.find(
	...
	const onSubmit = (data) => {
		...
		if (isRegisteredForVat === true) {
			if (data.vatRegistrationNumber === "") {
				setError('vatRegistrationNumber', { type: 'manual', message: 'Tax registration number is required' });
				return;
			}
			if (data.vatRegistrationNumber.length !== 15) {
				setError('vatRegistrationNumber', { type: 'manual', message: 'Please enter 15 digit Tax registration number' });
				return;
			}
		}
		if (trnExist === true) {
			setError('vatRegistrationNumber', { type: 'manual', message: 'Tax registration number already exists' });
			return;
		}
		if (emailExist === true) {
				...
				if (res.status === 200) {
					...
					if (createMore) {
						reset({
							...
							contactType: contactType || null,
							currencyCode: companyDetails ? currency_list_dropdown?.find(
											...
											<Form onSubmit={handleSubmit(onSubmit)}>
												...
												<Row className="row-wrapper">
													...
													{watchedValues.taxTreatmentId && watchedValues.taxTreatmentId.value && (
														<Col md="4" style={{ display: watchedValues.taxTreatmentId.value === 1 || watchedValues.taxTreatmentId.value === 3 || watchedValues.taxTreatmentId.value === 5 ? '' : 'none' }}>
															<FormGroup>
