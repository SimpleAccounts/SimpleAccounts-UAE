...
const ApplyToInvoice = () => {
    ...
    const initializeData = () => {
        if (location.state && location.state.contactId) {
            dispatch(CustomerInvoiceDetailActions.getInvoicesListForCN(location.state.contactId))
                .then((res) => {
                    if (res.status === 200) {
                                ...
                                <Form onSubmit={handleSubmit}>
                                    <Row>
                                        <Col lg={12} className='h5'>
                                            <span>{strings.CreditAmount}: <Currency value={location.state.creditAmount} currencySymbol={customer_currency} /></span>
                                        </Col>
                                        <Col lg={12} className='mb-1' style={{ fontSize: '12px', color: currenttotal > 0 ? 'Green' : 'red' }}>
                                            {strings.RemainingCredittAmount}: <Currency value={currenttotal} currencySymbol={customer_currency} /><br />
                                        </Col>
