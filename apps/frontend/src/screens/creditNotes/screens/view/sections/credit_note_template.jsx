...
const InvoiceTemplate = ({
	...
	const renderShippingPostZipCode = () => {
		...
		if (invoiceData.changeShippingAddress && invoiceData.changeShippingAddress == true) {
			...
			if (invoiceData.shippingCountry == 229) shippingPostZipCode = strings.POBox + ': ' + shippingPostZipCode;
		} else {
			if (contactData && contactData.isBillingAndShippingAddressSame && contactData.isBillingAndShippingAddressSame == true) {
				shippingPostZipCode = contactData.postZipCode ? contactData.postZipCode : '';
				if (contactData.shippingCountryId == 229) shippingPostZipCode = strings.POBox + ': ' + shippingPostZipCode;
			} else {
		...
		if (isBillingAndShippingAddressSame == false || invoiceData.changeShippingAddress == true)
			return (
				<div>
					...
					{contactData && (
						<div className="mb-1 ml-2">
							<b>{contactData.organization ? contactData.organization : contactData.firstName + ' ' + contactData.lastName}</b>
						</div>
					)}
		...
		<div>
			<Card id="singlePage" className="box">
				<CardBody style={{ margin: '1rem', border: 'solid 1px', borderColor: '#c8ced3' }}>
					...
					>
						<div style={{ width: '50%', marginTop: '4.5rem', marginLeft: '3rem' }}>
							<div className="companyDetails">
								<img
									src={companyData && companyData.companyLogoByteArray ? 'data:image/jpg;base64,' + companyData.companyLogoByteArray : logo}
									className=""
									alt=""
									style={{ width: ' 300px' }}
								/>
						...
						>
							...
							>
								<div style={{ marginTop: '0.5rem' }}>
									...
									<div className="mb-1 ml-2">
										{strings.MobileNumber}: {companyMobileNumber(companyData.phoneNumber ? '+' + companyData.phoneNumber : '')}
									</div>
					...
					>
						...
						>
							<div>
								...
								<div className="mb-1 ml-2">
									{invoiceData && contactData && (contactData.countryId == 229 ? (contactData.poBoxNumber ? strings.POBox + ': ' + contactData.poBoxNumber : '') : contactData.postZipCode ? contactData.postZipCode : '')}
									,&nbsp;
								...
								{invoiceData && invoiceData.taxTreatment && invoiceData.taxTreatment.includes('NON') == false && (
									<div className="mb-1 ml-2">
										{strings.VATRegistrationNo}: {contactData && contactData.vatRegistrationNumber && contactData.vatRegistrationNumber}
									</div>
								)}
								...
								<tr>
									...
									{isCNWithoutProduct && (
										<td style={{ textAlign: 'right' }}>
											<Currency value={invoiceData.totalAmount} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
										</td>
									)}
									<td style={{ textAlign: 'right' }}>
										<Currency value={invoiceData.dueAmount} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
									</td>
								</tr>
							</tbody>
						</Table>
							...
							<Table className="table-striped">
								<thead className="header-row" style={{ fontSize: '12px' }}>
									<tr>
										...
										{invoiceData.discount != 0 && <th style={{ padding: '0.5rem', textAlign: 'right' }}>{strings.Discount}</th>}
										...
										{invoiceData.totalVatAmount != 0 && <th style={{ padding: '0.5rem', textAlign: 'right' }}>{strings.VatAmount}</th>}
											...
											return (
												<tr key={index}>
													...
													<td style={{ textAlign: 'right', width: '10%' }}>
														<Currency value={item.unitPrice} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
													</td>
													{invoiceData.discount != 0 && (
														<>
															<td style={{ textAlign: 'right' }}>
																<Currency value={item.discount} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
															</td>
														</>
													)}
													{invoiceData.totalExciseTaxAmount != 0 && (
														<>
															<td style={{ textAlign: 'right' }}>{item.exciseTaxId ? renderExcise(item) : '-'}</td>
															<td style={{ textAlign: 'right' }}>
																{<Currency value={item.exciseAmount} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />}
															</td>
														</>
													)}
													{invoiceData.totalVatAmount != 0 && (
														<td style={{ textAlign: 'right' }}>
															<Currency value={item.vatAmount} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
														</td>
													)}
													<td style={{ textAlign: 'right' }}>
														<Currency value={item.subTotal} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
													</td>
												</tr>
											);
										})}
						...
						>
							...
							{invoiceData.notes && (
								<>
									...
									<br />
									<textarea className="mb-0" style={{ width: '500px', height: '200px', border: 'none' }} value={invoiceData.notes} readOnly />
								</>
							)}
						</div>
						<div
						...
						>
							<div style={{ width: '250px', marginLeft: 'auto' }}>
								<Table className="table-clear cal-table">
									<tbody>
										{isCNWithoutProduct == false && invoiceData.discount && invoiceData.discount > 0 ? (
											<tr>
												<td style={{ width: '40%' }}>
													<strong>
														{strings.Discount}
														{invoiceData.discountPercentage ? `(${invoiceData.discountPercentage}%)` : ''}
													</strong>
												</td>
												<td
													style={{
												...
												>
													...
													<span>
														{invoiceData.discount ? (
															<Currency value={invoiceData.discount ? +invoiceData.discount : invoiceData.discount} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
														) : (
															<Currency value={0} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
														)}
													</span>
												</td>
											</tr>
											...
											{!isCNWithoutProduct && (
												<>
													...
													<td style={{ display: 'flex', justifyContent: 'space-between' }}>
														...
														<span>
															{totalNet ? (
																<Currency value={totalNet - invoiceData.totalVatAmount - invoiceData.totalExciseTaxAmount} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
															) : (
																<Currency value={0} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
															)}
														</span>
												...
												>
													...
													<span>
														{invoiceData.totalExciseTaxAmount ? (
															<Currency value={invoiceData.totalExciseTaxAmount} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
														) : (
															<Currency value={0} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
														)}
													</span>
												...
												>
													...
													<span>
														{invoiceData.totalVatAmount ? (
															<Currency value={invoiceData.totalVatAmount} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
														) : (
															<Currency value={0} currencySymbol={currencyData[0] ? currencyData[0].currencyIsoCode : 'USD'} />
														)}
													</span>
					...
					<hr />
					<Input type="textarea" disabled className="textarea viewFootNote" maxLength="250" style={{ width: '1100px' }} value={invoiceData.footNote} />
					<br />
					<br />
					<br />
				</CardBody>
