...
class LogIn extends React.Component {
  ...
  getInitialData = () => {
    this.props.authActions
      .getCompanyCount()
      .then(response => {
        const companyCount = response?.data ?? 0;
        if (companyCount < 1) {
      ...
      .catch(err => {
        // If API fails (e.g., database not set up, 404, etc.), show register button
        console.log('getCompanyCount failed, allowing registration:', err);
  ...
  handleSubmit = (data, resetForm) => {
    ...
    this.props.authActions
      ...
      .catch(err => {
        ...
        toast.error(err && err.data ? 'Log in failed. ' : 'Something Went Wrong', {
          position: 'top-right',
        });
      });
  };
