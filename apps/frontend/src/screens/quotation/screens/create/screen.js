...
class CreateQuotation extends React.Component {
      ...
      <Field
        ...
        render={({ field, form }) => (
          <Input
            ...
            onChange={e => {
              this.selectItem(e.target.value, row, 'description', form, field, props);
            }}
          ...
          <Select
            ...
            value={
              ...
              selectOptionsFactory
                ...
                .find(option =>
                  row.exciseTaxId ? option.value === +row.exciseTaxId : 'Select Exise'
                )
            }
            id="exciseTaxId"
  ...
  getParentInvoiceDetails = parentId => {
    this.props.quotationCreateAction.getQuotationById(parentId).then(res => {
      if (res.status === 200) {
        ...
        this.setState(
          {
            ...
            initValue: {
              ...
              quotationdate: res.data.quotationdate
                ...
                : '',
              quotationdate1: res.data.quotationdate ? res.data.quotationdate : '',
              quotaionExpiration1: res.data.quotaionExpiration ? res.data.quotaionExpiration : '',
              customerId: res.data.customerId ? res.data.customerId : '',
              quotationNumber: res.data.quotationNumber ? res.data.quotationNumber : '',
              receiptNumber: res.data.receiptNumber ? res.data.receiptNumber : '',
              totalVatAmount: res.data.totalVatAmount ? res.data.totalVatAmount : 0,
            ...
            quotaionExpirationNotChanged: res.data.quotaionExpiration
              ...
              : '',
            quotaionExpiration: res.data.quotaionExpiration ? res.data.quotaionExpiration : '',
          ...
          () => {
            if (this.state.data.length > 0) {
              ...
              let customer = selectOptionsFactory
                .renderOptions('label', 'value', tmpSupplier_list, 'Customer Name')
              ...
              this.formRef.current.setFieldValue(
                'exchangeRate',
                res.data.exchangeRate ? res.data.exchangeRate : 1,
                true
              );
  ...
  getInitialData = () => {
    ...
    this.props.requestForQuotationAction.getVatList().then(res => {
      if (res.status == 200 && res.data) this.setState({ vat_list: res.data });
    });
    ...
    if (this.state.customer_taxTreatment_des) {
      ...
      if (product) {
        ...
        if (
          ...
          this.state.quotationDateForVatValidation > this.state.companyVATRegistrationDate
        ) {
          if (this.state.isDesignatedZone) {
            if (product.productType === 'GOODS') {
              if (
                this.state.customer_taxTreatment_des === 'UAE VAT REGISTERED' ||
                this.state.customer_taxTreatment_des === 'UAE VAT REGISTERED FREEZONE' ||
                this.state.customer_taxTreatment_des === 'UAE NON-VAT REGISTERED FREEZONE' ||
                this.state.customer_taxTreatment_des === 'GCC VAT REGISTERED' ||
                this.state.customer_taxTreatment_des === 'GCC NON-VAT REGISTERED' ||
              ...
              if (
                this.state.customer_taxTreatment_des === 'UAE VAT REGISTERED' ||
                this.state.customer_taxTreatment_des === 'UAE NON-VAT REGISTERED' ||
                this.state.customer_taxTreatment_des === 'UAE VAT REGISTERED FREEZONE' ||
                this.state.customer_taxTreatment_des === 'UAE NON-VAT REGISTERED FREEZONE'
              ) {
                vt = vat_list;
    ...
    this.setState(
      ...
      () => {
        ...
        data.map((obj, index) => {
          ...
          newData.push(obj);
          if (obj['productId']) this.getProductType(obj['productId']);
          return obj;
        });
  ...
  renderVat = (cell, row, props) => {
    ...
    if (row.productId && row.vatCategoryId) {
      row.vatCategoryId =
        typeof row.vatCategoryId === 'string' ? parseInt(row.vatCategoryId) : row.vatCategoryId;
    }
    return (
      <Field
        ...
        render={({ field, form }) => (
          <>
            <Select
              options={
                product_list
                  ? optionFactory.renderOptions('name', 'id', product_list, 'Product')
                  : []
              }
              id="productId"
  ...
  checkedRow = () => {
    if (this.state.data.length > 0) {
      ...
      let temp =
        this.state.data?.[length].productId !== '' ? this.state.data?.[length].productId : -2;
      if (temp > -1) {
        return true;
      } else {
        return false;
    ...
    data.map(obj => {
      ...
      if (this.state.taxType === false) {
        if (obj.discountType === 'PERCENTAGE') {
          net_value = (+obj.unitPrice - +(obj.unitPrice * obj.discount) / 100) * obj.quantity;
  ...
  getCurrentProduct = () => {
    this.props.requestForQuotationAction.getProductList().then(res => {
      ...
      this.formRef.current.setFieldValue(`lineItemsString.${0}.discount`, 1, true);
  ...
  validationCheck = value => {
    ...
    this.props.quotationCreateAction.checkValidation(data).then(response => {
  ...
  render() {
    ...
    const { currency_list, supplier_list, universal_currency_list, currency_convert_list } =
      this.props;
                          ...
                          <Formik
                            ...
                            validate={values => {
                              ...
                              if (this.state.exist === true) {
                                errors.quotation_Number = 'Quotation number already exists';
                              }
                                ...
                                if (
                                  ...
                                  (values.placeOfSupplyId == '' ||
                                    (values.placeOfSupplyId.label &&
                                      values.placeOfSupplyId.label === 'Select Place of Supply'))
                                )
                                  errors.placeOfSupplyId = 'Place of supply is required';
                              }
                              if (param === true) {
                              ...
                              if (!values.quotationdate) {
                                errors.quotationdate = strings.QuotationDateRequired;
                              }
                              if (
                                values.quotationdate &&
                                values.quotaionExpiration &&
                                values.quotationdate > values.quotaionExpiration
                              ) {
                            ...
                            validationSchema={Yup.object().shape({
                              quotation_Number: Yup.string().required('Invoice number is required'),
                              customerId: Yup.string().required(strings.Customer_Name_Is_Required),
                              // placeOfSupplyId: Yup.string().required('Place of supply is required'),

                              //  poApproveDate: Yup.string().required(
                              // 	'Order date is required',
                              // ),
                              quotaionExpiration: Yup.string().required('Expiry date is required'),
                              ...
                              <Form onSubmit={props.handleSubmit}>
                                      ...
                                      <Input
                                        ...
                                        onChange={option => {
                                          if (
                                            ...
                                            props.handleChange('quotation_Number')(option);
                                          }
                                ...
                                <Row>
                                  ...
                                  {this.state.customer_taxTreatment_des && isRegisteredVat && (
                                    <Col lg={3}>
                                      <FormGroup className="mb-3">
                                        <Label htmlFor="taxTreatmentid">Tax Treatment</Label>
                                        <Input
                                          disabled
                                          styles={customStyles}
                                          id="taxTreatmentid"
                                            ...
                                            value={
                                              ...
                                              selectOptionsFactory
                                                ...
                                                .find(
                                                  option =>
                                                    option.value ==
                                                    (this.state.quotationId || this.state.parentId
                                  ...
                                  <Col lg={3}>
                                    <FormGroup className="mb-3">
                                      ...
                                      <DatePicker
                                        ...
                                        name="quotationdate"
                                        className={`form-control ${
                                          props.errors.quotationdate && props.touched.quotationdate
                                            ? 'is-invalid'
                                            : ''
                                        }`}
                                        placeholderText={strings.Select + strings.QuotationDate}
                                        selected={
                                          props.values.quotationdate
                                            ? new Date(props.values.quotationdate)
                                            : props.values.quotationdate
                                        }
                                        showMonthDropdown
                                        showYearDropdown
                                  ...
                                  <Col lg={3}>
                                    <FormGroup className="mb-3">
                                      ...
                                      <DatePicker
                                        ...
                                        placeholderText={strings.Select + strings.ExpirationDate}
                                        selected={
                                          props.values.quotaionExpiration
                                            ? new Date(props.values.quotaionExpiration)
                                            : props.values.quotaionExpiration
                                        }
                                        showMonthDropdown
                                        showYearDropdown
                                      ...
                                      {props.errors.quotaionExpiration &&
                                        props.touched.quotaionExpiration && (
                                          <div className="invalid-feedback">
                                            {/* {props.errors.quotaionExpiration} */}
                                            {props.errors.quotaionExpiration.includes('nullable()')
                                              ? strings.ExpiryDateRequired
                                              : props.errors.quotaionExpiration}
                                          </div>
                                        )}
                                    </FormGroup>
                                      ...
                                      <Select
                                        ...
                                        value={
                                          ...
                                          selectCurrencyFactory
                                            ...
                                            .find(
                                              option =>
                                                option.value === this.state.supplier_currency
                                            )
                                        }
                                        onChange={option => {
                                    ...
                                    <Col lg={8}>
                                      ...
                                      <FormGroup className="mb-3 hideAttachment">
                                        ...
                                        <TextField
                                          ...
                                          onChange={option =>
                                            props.handleChange('attachmentDescription')(option)
                                          }
                                            ...
                                            <Row>
                                              ...
                                              <Col lg={6} className="text-right">
                                                <label className="mb-0">
                                                  ...
                                                  {this.state.supplier_currency_symbol}&nbsp;
                                                  {initValue.invoiceVATAmount.toLocaleString(
                                                    navigator.language,
                                                    {
                                                      minimumFractionDigits: 2,
                                                      maximumFractionDigits: 2,
                                    ...
                                    <FormGroup className="text-right">
                                      ...
                                      <Button
                                        ...
                                        onClick={() => {
                                          ...
                                          } else this.props.history.push('/admin/income/quotation');
                                        }}
                                      >
