...
class ViewQuotation extends React.Component {
  ...
  initializeData = () => {
    ...
    if (this.props.location.state && this.props.location.state.id) {
      this.props.quotationDetailsAction.getQuotationById(this.props.location.state.id).then(res => {
        if (res.status === 200) {
  ...
  render() {
    const { QuotationData, currencyData, id, contactData, actionList, invoiceStatus, invoiceData } =
      this.state;
