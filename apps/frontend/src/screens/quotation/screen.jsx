...
const Quatation = () => {
    ...
    const columns = useMemo(() => [
            ...
            cell: ({ row }) => (
                <div>
					<div>
						...
						<label>
							{row.original.totalAmount  === 0 ? row.original.currencyIsoCode +" "+ row.original.totalAmount.toLocaleString(navigator.language, { minimumFractionDigits: 2,maximumFractionDigits: 2 }) :  row.original.currencyIsoCode +" "+ row.original.totalAmount.toLocaleString(navigator.language, { minimumFractionDigits: 2,maximumFractionDigits: 2 })}
						</label>
					</div>
					...
					<div style={{display: row.original.totalVatAmount === 0 ? 'none' : ''}}>
						...
						<label>
							{row.original.totalVatAmount === 0  ?  row.original.currencyIsoCode +" "+ row.original.totalVatAmount.toLocaleString(navigator.language, { minimumFractionDigits: 2,maximumFractionDigits: 2 }) :   row.original.currencyIsoCode +" "+ row.original.totalVatAmount.toLocaleString(navigator.language, { minimumFractionDigits: 2,maximumFractionDigits: 2 })}
						</label>
        ...
        },
        {
            ...
            header: '',
            cell: ({ row }) => {
                const statuslist = StatusActionList.QuotationStatusActionList.find(obj => obj.status === row.original.status);
							...
							<Row>
								<Col lg={12}>
									...
									<Button
										...
										onClick={() =>
											navigate(`/admin/income/quotation/create`)
										}
									>
