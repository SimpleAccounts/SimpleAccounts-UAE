	...
	getColumns = () => {
		return [
			...
			},
			{
				...
				cell: ({ row }) => (
					<div style={{ whiteSpace: 'normal' }}>{row.original.customerName}</div>
				),
				enableSorting: true,
			},
			{
					...
					return (
						<div style={{ textAlign: 'right' }}>
							<div>
								...
								<label>
									{currencySymbol} {invoiceAmount.toLocaleString(navigator.language, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
								</label>
							</div>
							{vatAmount !== 0 && (
								<div>
									<label className="font-weight-bold mr-2">{strings.VatAmount}: </label>
									<label>{currencySymbol} {vatAmount.toLocaleString(navigator.language, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</label>
								</div>
							)}
							{statusEnum !== 'Paid' && dueAmount !== 0 && (
								<div>
									<label className="font-weight-bold mr-2">{strings.DueAmount}: </label>
									<label>{currencySymbol} {dueAmount.toLocaleString(navigator.language, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</label>
								</div>
							)}
						</div>
					);
				...
				cell: ({ row }) => {
					...
					return (
						...
						>
							...
							<DropdownMenu right>
								{rowData.statusEnum !== 'Paid' && rowData.statusEnum !== 'Sent' && rowData.statusEnum !== 'Partially Paid' && (
									<DropdownItem
										onClick={() => {
											if (rowData.editFlag)
								...
								{rowData.statusEnum !== 'Sent' && rowData.statusEnum !== 'Paid' && rowData.statusEnum !== 'Partially Paid' && (
									<DropdownItem
										onClick={() => {
								...
								<DropdownItem
									onClick={() =>
										this.props.history.push('/admin/expense/supplier-invoice/create', { parentInvoiceId: rowData.id })
									}
								>
									<i className="fas fa-copy" /> {strings.CreateADuplicate}
								</DropdownItem>
								{(!rowData.cnCreatedOnPaidInvoice && rowData.statusEnum === 'Paid' && config.EXPENSE_DN) && (
									<DropdownItem
										onClick={() => {
											this.props.history.push('/admin/expense/debit-notes/create', {
												invoiceID: rowData.id, invoiceNumber: rowData.invoiceNumber,
											})
										}}
									>
								...
								{rowData.statusEnum !== 'Draft' && rowData.statusEnum !== 'Paid' && rowData.exchangeRate === 1 && (
									<DropdownItem
										onClick={() => {
											this.props.history.push(
								...
								<DropdownItem
									onClick={() =>
										this.props.history.push(
											...
											{ id: rowData.id, status: rowData.status },
										)
									}
								>
