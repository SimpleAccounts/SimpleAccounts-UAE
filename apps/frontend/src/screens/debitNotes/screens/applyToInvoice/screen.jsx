...
import { z } from 'zod';
import { Card, CardHeader, CardBody, Button, Row, Col, Form, FormGroup } from 'components/migration';
...
const ApplyToSupplierInvoice = () => {
  ...
  const initializeData = () => {
    if (location.state && location.state.contactId) {
      dispatch(DebitNoteApplyToInvoiceActions.getInvoicesListForCN(location.state.contactId)).then(
        res => {
          if (res.status === 200) {
                  ...
                  <Row>
                    <Col lg={12} className="h5">
                      <span>
                        {strings.DebitAmount}:{' '}
                        <Currency
                          value={location.state.debitAmount}
                    ...
                    >
                      {strings.RemainingDebitAmount}:{' '}
                      <Currency value={currentTotal} currencySymbol={customerCurrency} />
                      <br />
                    </Col>
                    <Col lg={12}>
                      <DataTable
