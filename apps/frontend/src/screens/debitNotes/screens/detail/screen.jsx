...
const DetailDebitNote = () => {
	...
	const initializeData = () => {
		...
		if (location.state && location.state.id) {
			dispatch(DebitNotesActions.getDebitNoteById(location.state.id, location.state.isCNWithoutProduct || false))
				.then(res => {
					if (res.status === 200) {
                        const r = res.data;
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <Select
                    ...
                    value={{ value: row.original.transactionCategoryId, label: row.original.transactionCategoryLabel }}
                />
            ),
        },
        {
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <Select
                    ...
                    value={vat_list && selectOptionsFactory.renderOptions('name', 'id', vat_list, 'Vat').find(opt => opt.value === +row.original.vatCategoryId)}
                />
            ),
        },
        {
            ...
            cell: ({ row }) => <div className="text-right">{customer_currency_symbol} {row.original.subTotal?.toFixed(2)}</div>,
        },
