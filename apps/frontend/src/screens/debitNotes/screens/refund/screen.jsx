...
const debitNoteRefundSchema = z.object({
	...
	depositeTo: z.union([
		...
		z.object({
			...
			label: z.string(),
		}).refine((val) => val.value !== '' && val.value !== null && val.value !== undefined, { message: 'Deposit to is required' }),
	]),
...
const DebitNoteRefund = (props) => {
	...
	const {
		...
		defaultValues: {
			...
			debitNoteDate: location.state.id.creditNoteDate ? new Date(dayjs(location.state.id.creditNoteDate, 'DD-MM-YYYY').format()) : '',
	...
	const onSubmit = (data) => {
		...
		if (isCNWithoutProduct == true) {
			...
			debitNotesRefundActions.refundPaymentCNWithoutInvoice(formData)
				.then((res) => {
			...
			debitNotesRefundActions.refundPaymentCNWithInvoice(formData)
				.then((res) => {
					commonActions.tostifyAlert('success', strings.RefundRecordedSuccessfully);
					history.push('/admin/expense/debit-notes');
					setLoading(false);
																	...
																	<Select
																		...
																		value={
																			...
																			tmpcustomer_list.find(
																				(option) =>
																					option.value === +location.state.id.contactId,
																			)
																		}
																		className={
															...
															<Controller
																...
																render={({ field }) => (
																	<DatePicker
																		...
																		className={`form-control ${errors.paymentDate && touchedFields.paymentDate
																			? 'is-invalid'
																			: ''
																			}`}
																	/>
															...
															<Controller
																...
																render={({ field }) => (
																	<Input
																		...
																		className={errors.receiptNumber && touchedFields.receiptNumber ? "is-invalid" : " "}
																	/>
																)}
															/>
															{errors.receiptNumber && touchedFields.receiptNumber && (
																<div className="invalid-feedback">{errors.receiptNumber.message}</div>
															)}
														</FormGroup>
													</Col>
												</Row>
