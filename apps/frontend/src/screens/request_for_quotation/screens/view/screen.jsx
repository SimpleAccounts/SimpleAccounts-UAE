...
const ViewRequestForQuotation = (props) => {
	...
	const initializeData = () => {
		...
		if (props.location.state && props.location.state.id) {
			props.requestForQuotationDetailsAction
				.getRFQeById(props.location.state.id)
				.then((res) => {
					let val = 0;
					if (res.status === 200) {
						...
						if (res.data.supplierId) {
							props.requestForQuotationViewAction
								.getContactById(res.data.supplierId)
								.then((res) => {
									if (res.status === 200) {
			...
			props.requestForQuotationViewAction
				.getPoGrnById(props.location.state.id)
				.then((res) => {
					if (res.status === 200) {
