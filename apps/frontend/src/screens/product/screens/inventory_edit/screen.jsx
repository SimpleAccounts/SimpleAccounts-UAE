...
const InventoryEdit = ({
  ...
  const initializeData = () => {
    if (location.state && location.state.id) {
      ...
      inventoryAccountFn();
      productActions.getInventoryById(location.state.id).then(res => {
        if (res.status === 200) {
          setLoading(false);
  ...
  const deleteProduct = () => {
    productActions.getInvoicesCountProduct(currentInventoryId).then(res => {
      if (res.data > 0) {
                            ...
                            <Row>
                              <Col lg={4}>
                                <FormGroup className="mb-3">
                                  <Label htmlFor="salesUnitPrice">{strings.InventoryAccount}</Label>
                                  <Controller
                                      ...
                                      <Select
                                        ...
                                        value={
                                          ...
                                          inventoryAccount.find(
                                            option =>
                                              option.value === +watchedValues.transactionCategoryId
                                          )
                                        }
                              ...
                              <Col lg={4}>
                                <FormGroup className="mb-3">
                                  <Label htmlFor="inventoryQty">{strings.OpeningStock}</Label>
                                  <Controller
                                    name="inventoryQty"
                                    control={control}
                                    render={({ field }) => (
                                      ...
                                      <Select
                                        ...
                                        value={
                                          ...
                                          tmpSupplier_list.find(
                                            option => option.value === +watchedValues.contactId
                                          )
                                        }
                                        onChange={option => {
                                          field.onChange(option ? option.value : '');
