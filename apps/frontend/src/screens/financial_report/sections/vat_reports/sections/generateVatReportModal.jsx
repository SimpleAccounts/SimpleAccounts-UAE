  ...
  const getEndDate = () => {
    if (monthlyDate) {
      if (monthOption.value === 0) {
        return dayjs(monthlyDate).add(1, 'month').subtract(1, 'day').format('DD-MM-YYYY');
      } else if (monthOption.value === 2) {
        return dayjs(monthlyDate).add(3, 'month').subtract(1, 'day').format('DD-MM-YYYY');
      }
    }
    return '';
  };
  ...
  const generateReport = () => {
    ...
    vatReportDataList.data.map(({ taxReturns }) => {
      ...
      if (
        ...
        (currentStartDate.diff(startDate, 'days') >= 0 && currentEndDate.diff(endDate, 'days') <= 0)
      ) {
        notGenerated = false;
      }
    });
      ...
      .then(res => {
        if (res.status === 200) {
          props.commonActions.tostifyAlert(
            'success',
            res.data && res.data.message ? res.data.message : 'VAT Report Generated Successfully'
          );
        }
        closeModal(false);
      })
