        ...
        if (res.status === 200) {
          ...
          res.data.creditNoteSummaryModelList.map((row, i) => {
            if (row.type === 7) {
              ...
              row.creditNoteDate = row.creditNoteDate
                ? dayjs(row.creditNoteDate).format('DD-MM-YYYY')
                : '';
...
export default connect(mapStateToProps, mapDispatchToProps)(CreditNoteDetailsReport);
