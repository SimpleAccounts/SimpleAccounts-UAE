...
const createValidationSchema = (isTANMandatory) => {
	return z.object({
		taxablePersonNameInEnglish: z.string()
			.optional()
			.refine((val) => !val || regExAlpha.test(val), {
				message: "A taxable person's name must contain only alphabets",
			}),
		taxablePersonNameInArabic: z.string()
			.optional()
			.refine((val) => !val || true, { // Arabic has different character set
				message: "A taxable person's name must contain only alphabets",
			}),
		vatRegistrationNumber: z.string().optional(),
		taxAgencyName: z.string()
			.optional()
			.refine((val) => !val || regExAlpha.test(val), {
		...
		taxAgentName: z.string()
			.optional()
			.refine((val) => !val || regExAlpha.test(val), {
				message: "Tax agent name must contain only alphabets",
			}),
		taxAgentApprovalNumber: z.string()
			.optional()
...
const VatSettingModal = ({ openModal, closeModal }) => {
	...
	const exciseTaxAuditReportActions = bindActionCreators(ExciseTaxAuditReportActions, dispatch);
	...
	useEffect(() => {
		exciseTaxAuditReportActions.getCompanyDetails().then((res) => {
			if (res.status === 200) {
				reset({
