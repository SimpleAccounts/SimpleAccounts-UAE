...
const EntryCellRenderer = ({ value, data, getInvoice }) => {
  if (!data || !getInvoice) {
    return value;
  }
...
const AmountCellRenderer = ({ value, data }) => {
  if (value == null || value === 0) {
    return '---';
  }

  return <Currency value={value} currencySymbol={data?.currency ? data.currency : 'AED'} />;
};
    ...
    const columnDefs = [
      {
        ...
        filter: true,
        enablePivot: true,
        valueFormatter: params => this.renderDate(params.value, params.data),
      },
      {
        field: 'entry',
        headerName: 'Entry #',
        ...
        cellRendererParams: { getInvoice: this.getInvoice },
      },
      {
        field: 'amount',
        headerName: 'Amount',
                  ...
                  >
                    ...
                    <div className="d-flex">
                      <Button
                        ...
                        onClick={() => {
                          this.props.history.push(`/admin/report/vatreports/view?id=${id}`, {
                            startDate: this.props.location.state.startDate,
                            endDate: this.props.location.state.endDate,
                          });
                        }}
            ...
            <CardBody>
              ...
              >
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <br />
                  From {this.props.location.state.startDate.replaceAll('/', '-')}&nbsp; To{' '}
                  {this.props.location.state.endDate.replaceAll('/', '-')}
                </div>
              </div>
