        ...
        switch (postingType) {
          case 'INVOICE':
            if (type === 1) {
              navigate('/admin/expense/supplier-invoice/view', {
                state: { id, boxNo, description, startDate, endDate, placeOfSupplyId, crossLinked: true }
              });
            } else {
          ...
          case 'EXPENSE':
            navigate('/admin/expense/expense/view', {
              state: { expenseId: id, boxNo, description, startDate, endDate, placeOfSupplyId, crossLinked: true }
            });
            break;
          case 'MANUAL':
				...
				<Card>
					...
					<CardBody>
						<div className="flex justify-between mb-4">
							<img src={company_profile?.companyLogoByteArray ? `data:image/jpg;base64,${company_profile.companyLogoByteArray}` : logo} alt="logo" style={{ width: '150px' }} />
							<div className="text-center">
								<h2>{company_profile?.companyName || 'ABC GROUP'}</h2>
								<b className="text-lg">{location.state.description}</b>
								<br />
								From {location.state.startDate?.replaceAll('/', '-')} To {location.state.endDate?.replaceAll('/', '-')}
							</div>
