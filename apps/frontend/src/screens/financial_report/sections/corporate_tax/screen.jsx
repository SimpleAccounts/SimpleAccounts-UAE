...
const CorporateTax = () => {
  ...
  const renderStatus = (params) => {
    ...
    return (
      <>
        {status === 'UnFiled' ? (
          <label className="badge label-draft"> {status}</label>
        ) : (
          ''
        )}
        {status === 'Filed' ? (
          <label className="badge label-due"> {status}</label>
        ) : (
          ''
        )}
        ...
        {status === 'Paid' ? (
          <label className="badge label-paid">{status}</label>
        ) : (
          ''
        )}
        {status === 'claimed' ? (
        ...
        {status === 'Reclaimed' ? (
          <label className="badge label-sent"> {status}</label>
        ) : (
          ''
        )}
      </>
        ...
        <ShadcnDropdownMenuContent align="end">
          <ShadcnDropdownMenuItem
            onClick={() => {
              ...
              navigate(
                `/admin/report/corporate-tax/view?id=${row.id}`,
                {
                  state: {
                    id: row.id,
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => <div className="text-right">{renderAmount(row.original.netIncome, row.original.currency)}</div>,
    },
    {
      accessorKey: 'taxableAmount',
      header: strings.TaxableAmount,
      cell: ({ row }) => <div className="text-right">{renderAmount(row.original.taxableAmount, row.original.currency)}</div>,
    },
    {
      accessorKey: 'taxAmount',
      header: strings.TaxAmount,
      cell: ({ row }) => <div className="text-right">{renderAmount(row.original.taxAmount, row.original.currency)}</div>,
    },
    {
            ...
            <CardBody>
              ...
              <div>
                <DataTable
                  ...
                  pageCount={ctReport_list?.count ? Math.ceil(ctReport_list.count / pagination.pageSize) : 0}
