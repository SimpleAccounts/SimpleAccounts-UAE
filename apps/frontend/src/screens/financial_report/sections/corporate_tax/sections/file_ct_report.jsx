  ...
  const handleFormSubmit = data => {
    ...
    props.ctReportActions
      ...
      .then(res => {
        if (res.status === 200) {
          setDisabled(false);
          props.commonActions.tostifyAlert('success', 'Report Filed Successfully!');
        }
        closeModal(false);
      })
      .catch(err => {
        ...
        <Form onSubmit={rhfHandleSubmit(onSubmit)} className="create-contact-screen">
          <ModalBody>
            <Row className="mb-4">
              <Col>
                <h4>
                  Transactions for the tax period cannot be edited after the tax report has been
                  filed.
                </h4>
              </Col>
            </Row>
            ...
            <Row>
              <Col lg={4}>
                <FormGroup className="mb-3">
                  ...
                  />
                  {errors.taxFiledOn && (
                    <div className="invalid-feedback">{errors.taxFiledOn.message}</div>
                  )}
                </FormGroup>
              </Col>
            </Row>
...
export default connect(mapStateToProps, mapDispatchToProps)(FileCtReportModal);
