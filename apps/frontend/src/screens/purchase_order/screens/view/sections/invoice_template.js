...
class RFQTemplate extends Component {
  ...
  renderVat = POData => {
    ...
    if (
      ...
      POData.poQuatationLineItemRequestModelList.length != 0
    ) {
              ...
              >
                <div>
                  <h6 className="mb-1 ml-2">
                    <b>{strings.PurchaseFrom},</b>
                  </h6>
                  <br />
                  <div className="mb-1 ml-2">
                    <b>{POData.organisationName ? POData.organisationName : POData.supplierName}</b>
                  </div>
                  ...
                  {POData &&
                    POData.taxtreatment &&
                    POData.taxtreatment.includes('NON') == false && (
                      <div className="mb-1 ml-2">
                        {strings.VATRegistrationNo}: {POData.vatRegistrationNumber}
                      </div>
                    )}
                ...
                <div style={{ width: '27%' }}>
                  <br />
                  <div className="mb-1 ml-2">
                    <b>{strings.PONo}: </b> # {POData.poNumber}
                  </div>
                  {POData.receiptNumber && (
                    <div className="mb-1 ml-2">
                  ...
                  <div className="mb-1 ml-2">
                    ...
                    {dayjs(POData.poApproveDate).format('DD MMM YYYY')}
                  </div>
                  <div className="mb-1 ml-2">
                    <b>{strings.ReceiveDate}: </b>
                    {dayjs(POData.poReceiveDate).format('DD MMM YYYY')}
                  </div>
                  <div className="mb-1 ml-2">
                    <b>{strings.Status}: </b>
                    {this.renderRFQStatus(status)}
                  </div>
                </div>
              </div>
            </div>
                      ...
                      <tr key={index}>
                        ...
                        {POData.totalExciseAmount > 0 && (
                          <>
                            ...
                            <td style={{ textAlign: 'right' }}>
                              {POData.totalExciseAmount
                                ? POData.currencyIsoCode +
                                  ' ' +
                                  POData.totalExciseAmount.toLocaleString(navigator.language, {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                  })
                                : POData.currencyIsoCode +
              ...
              >
                <div style={{ width: '100%' }}>
                  <Table className="table-clear cal-table">
                    <tbody>
                      {POData.totalExciseAmount && POData.totalExciseAmount > 0 ? (
                        <tr>
                          <td style={{ width: '40%' }}>
                      ...
                      {POData.discount && POData.discount > 0 ? (
                        <tr>
                          <td style={{ width: '40%' }}>
                            <strong>
                              {strings.Discount}
                              {POData.discountPercentage ? `(${POData.discountPercentage}%)` : ''}
                            </strong>
                          </td>
                          <td
                            style={{
                          ...
                          >
                            ...
                            <span>
                              {POData.discount
                                ? POData.currencyIsoCode +
                                  ' ' +
                                  POData.discount.toLocaleString(navigator.language, {
                                    minimumFractionDigits: 2,
                                  })
                                : POData.currencyIsoCode +
                                  ' ' +
                            ...
                            {POData.totalAmount
                              ? POData.currencyIsoCode +
                                ...
                                (
                                  ...
                                  parseFloat(POData.totalVatAmount) -
                                  parseFloat(POData.totalExciseAmount)
                        ...
                        >
                          ...
                          <span>
                            {POData
                              ? POData.currencyIsoCode +
                                ' ' +
                                this.renderVat(POData).toLocaleString(navigator.language, {
                                  minimumFractionDigits: 2,
                                })
                              : POData.currencyIsoCode +
                                ' ' +
                        ...
                        >
                          ...
                          <span>
                            {POData.totalAmount
                              ? POData.currencyIsoCode +
                                ' ' +
                                POData.totalAmount.toLocaleString(navigator.language, {
                                  minimumFractionDigits: 2,
                                })
                              : 0}
                          </span>
