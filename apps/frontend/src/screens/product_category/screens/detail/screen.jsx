...
const mapDispatchToProps = (dispatch) => {
	return {
		...
		detailProductCategoryAction: bindActionCreators(
...
const DetailProductCategory = ({
					...
					if (res.status === 200) {
						...
						reset({
							...
							productCategoryCode: res.data.productCategoryCode
								? res.data.productCategoryCode
								: '',
							productCategoryName: res.data.productCategoryName
								? res.data.productCategoryName
								: '',
						});
	...
	const deleteProductCategory = () => {
		...
		const message =
			'This Product Category will be deleted permanently and cannot be recovered. ';
		setDialog(
			<ConfirmDeleteModal
				isOpen={true}
