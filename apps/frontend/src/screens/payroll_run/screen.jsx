...
const PayrollRun = () => {
    ...
    const goToDetail = (row) => {
        const userValue = user_approver_generater_dropdown_list?.length ? user_approver_generater_dropdown_list[0].value : '';
        const userLabel = user_approver_generater_dropdown_list?.length ? user_approver_generater_dropdown_list[0].label : '';
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            cell: ({ row, getValue }) => (
                <label className="mb-0 label-bank cursor-pointer">
                    {getValue()}
                </label>
            ),
        },
        ...
        },
        {
            ...
            cell: ({ getValue }) => {
                ...
                else if (status === 'Paid') classname = 'label-sent'; // Assuming Paid is sent logic from original
            ...
            cell: ({ row }) => (
                <div className="text-right">
                    <div>
                        ...
                        <label>
                            {row.original.totalAmountPayroll === 0 ? "AED " + row.original.totalAmountPayroll.toLocaleString(navigator.language, { minimumFractionDigits: 2 }) : "AED " + row.original.totalAmountPayroll.toLocaleString(navigator.language, { minimumFractionDigits: 2 })}
                        </label>
                    </div>
                    {row.original.dueAmountPayroll != null && (
                        <div style={{ display: row.original.dueAmountPayroll === 0 ? 'none' : '' }}>
                            ...
                            <label>{row.original.dueAmountPayroll === 0 ? row.original.dueAmountPayroll + " " + row.original.dueAmountPayroll.toLocaleString(navigator.language, { minimumFractionDigits: 2 }) : "AED " + row.original.dueAmountPayroll.toLocaleString(navigator.language, { minimumFractionDigits: 2 })}</label>
                        </div>
                    )}
                </div>
            ),
                                    ...
                                    <Row className="mb-4 ">
                                        {userForCheckApprover !== "Payroll Approver" && userForCheckApprover !== "Accountant" && (
                                            <Col>
                                                <Button
                                                    color="primary"
                                                    title={disableCreatePayroll && sifEnabled ? "Please Create Company Details" : ""}
