...
const UpdatePayroll = () => {
	...
	const proceed = (payroll_id) => {
		dispatch(CreatePayrollActions.getPayrollById(payroll_id)).then((res) => {
			if (res.status === 200) {
				...
				getAllPayrollEmployee(dayjs(dateArray[0], 'DD/MM/YYYY'), res.data.status, res.data.existEmpList);
				calculatePayperiod(
		...
		if (pid) {
			...
			dispatch(EmployeeActions.getEmployeeListWithDetails()).then((response) => {
				if (response.status === 200) {
					...
					dispatch(CreatePayrollActions.getAllPayrollEmployee2(pid, dayjs(date).format('DD/MM/YYYY')))
						.then((res) => {
							if (res.status === 200) {
                                let dataToProcess = res.data;
								if (res.data.length === 0) {
									dispatch(CreatePayrollActions.getAllPayrollEmployee(pid, date))
										.then((res2) => {
											if (res2.status === 200) {
                                                processData(res2.data, employeePayPeriodlList, date, currentStatus, existingEmployees);
											}
										});
								} else {
                                    processData(res.data, employeePayPeriodlList, date, currentStatus, existingEmployees);
								}
							}
						});
				}
    ...
    const processData = (rawData, employeePayPeriodlList, date, currentStatus, existingEmployees) => {
        ...
        if (currentStatus && currentStatus === 'Submitted') {
            dispatch(CreatePayrollActions.getAllPayrollEmployeeForApprover(payrollId))
                .then((res) => {
                    if (res.status === 200) {
                        finalData = res.data;
