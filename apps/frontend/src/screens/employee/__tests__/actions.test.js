...
describe('Employee Actions', () => {
  ...
  describe('getEmployeeList', () => {
    ...
    it('should handle fetch error', async () => {
      ...
      await expect(store.dispatch(actions.getEmployeeList({}))).rejects.toThrow('Network error');
    });
  ...
  describe('removeBulkEmployee', () => {
    ...
    it('should handle bulk delete error', async () => {
      ...
      await expect(store.dispatch(actions.removeBulkEmployee({ ids: [1, 2] }))).rejects.toThrow(
        'Delete failed'
      );
    });
    ...
    it('should only return response on status 200', async () => {
      ...
      const result = await store.dispatch(actions.removeBulkEmployee({ ids: [1] }));
    ...
    it('should handle server validation errors', async () => {
      authApi.mockRejectedValue(new Error('Cannot delete employees with active payroll'));

      await expect(store.dispatch(actions.removeBulkEmployee({ ids: [1, 2, 3] }))).rejects.toThrow(
        'Cannot delete employees with active payroll'
      );
    });
  });
});
