		...
		if (location.state && location.state.id) {
			...
			detailCurrencyConvertAction
				...
				.then((res) => {
					if (res.status === 200) {
						detailCurrencyConvertAction
							.getDeleteStatusById(res.data.currencyCode)
							.then((res) => {
		...
		detailCurrencyConvertAction
			...
			.catch((err) => {
				commonActions.tostifyAlert(
					...
					err && err.data
						? err.data.message
						: 'Currency Conversion Deleted Unsuccessfully',
				);
			});
	};
										...
										<Col lg={10}>
											<Form onSubmit={handleSubmit(onSubmit)} name="simpleForm">
															...
															<Controller
																...
																render={({ field: { onChange, value, ...field } }) => (
																	<Input
																		...
																		id="exchangeRate"
																		placeholder={
																			strings.Enter + strings.Exchangerate
																		}
																		onChange={(e) => {
																			if (
																				e.target.value === '' ||
																		...
																		value={value}
																		className={
																			errors.exchangeRate ? 'is-invalid' : ''
																		}
																	/>
																)}
															/>
												...
												<span style={{ fontWeight: 'bold' }}>
													Note: If a currency is associated with any bank, contact
													or document, it cannot be deleted.
												</span>
												<Row>
													<Col
														lg={10}
