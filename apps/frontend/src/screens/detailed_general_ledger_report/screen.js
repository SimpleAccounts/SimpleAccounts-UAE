...
class DetailedGeneralLedgerReport extends React.Component {
  constructor(props) {
    ...
    this.columnHeader = [
      ...
      {
        label: strings1.Transaction + ' ' + strings1.Details,
  ...
  initializeData = () => {
    ...
    this.props.detailGeneralLedgerActions
      ...
      .then(res => {
        if (res.status === 200) {
          ...
          this.setState({
            detailedGeneralLedgerReportList: detailedGeneralLedgerList,
            loading: false,
          });
        }
      })
    ...
    this.props.detailGeneralLedgerActions.getTransactionCategoryList(postData).then(res => {
      if (res.status === 200) {
        this.setState({
  ...
  getList = detailedGeneralLedgerList => {
    ...
    let id = 0;
    detailedGeneralLedgerList &&
      detailedGeneralLedgerList.length > 0 &&
      detailedGeneralLedgerList.map(item => {
        ...
        item.map(row => {
          const detailedGeneralLedgerDetails = {
            ...
            transactionTypeName:
              row.postingReferenceTypeEnum !== 'Opening Balance' ? row.transactionTypeName : '',
            name: row.name,
            transactionRefNo:
    ...
    const sortedData = this.state.detailedGeneralLedgerList.map(data => {
      ...
      data.sort((a, b) => {
        if (column !== 'date') {
          ...
          nameA = a[`${column}`] ? dayjs(a[`${column}`], 'DD/MM/YYYY').toDate() : '';
          nameB = b[`${column}`] ? dayjs(b[`${column}`], 'DD/MM/YYYY').toDate() : '';
        }
        if (nameA < nameB) {
          return -1;
        }
