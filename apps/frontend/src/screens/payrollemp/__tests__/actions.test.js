...
describe('PayrollEmployee Actions', () => {
  ...
  describe('getPayrollEmployeeList', () => {
    it('should fetch payroll employee list successfully', async () => {
      ...
      const result = await store.dispatch(actions.getPayrollEmployeeList(params));
    ...
    it('should handle fetch error', async () => {
      ...
      await expect(store.dispatch(actions.getPayrollEmployeeList({}))).rejects.toThrow(
        'Network error'
      );
    });
    ...
    it('should dispatch correct action type', async () => {
      ...
      const dispatchedActions = store.getActions();
      expect(dispatchedActions[0].type).toBe(EMPLOYEEPAYROLL.PAYROLL_EMPLOYEE_LIST);
    });
  ...
  describe('getPayrollEmployeeList2', () => {
    it('should fetch active employees successfully', async () => {
      ...
      const result = await store.dispatch(actions.getPayrollEmployeeList2(params));
    ...
    it('should not dispatch when paginationDisable is true', async () => {
      ...
      await store.dispatch(actions.getPayrollEmployeeList2({ paginationDisable: true }));
    ...
    it('should handle fetch error for active employees', async () => {
      ...
      await expect(store.dispatch(actions.getPayrollEmployeeList2({}))).rejects.toThrow(
        'Database error'
      );
    });
    ...
    it('should handle name filter for active employees', async () => {
      ...
      await store.dispatch(actions.getPayrollEmployeeList2({ name: 'Active' }));
    ...
    it('should handle pagination for active employees', async () => {
      ...
      await store.dispatch(actions.getPayrollEmployeeList2({ pageNo: 3, pageSize: 15 }));
    ...
    it('should handle sorting for active employees', async () => {
      ...
      await store.dispatch(actions.getPayrollEmployeeList2({ order: 'desc', sortingCol: 'name' }));
  ...
  describe('Multiple concurrent requests', () => {
    it('should handle concurrent API calls', async () => {
      ...
      authApi.mockResolvedValueOnce(mockResponse1).mockResolvedValueOnce(mockResponse2);
