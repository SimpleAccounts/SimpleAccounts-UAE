...
class SalaryComponentDeduction extends React.Component {
  ...
  render() {
    const { openSalaryComponentDeduction, closeSalaryComponentDeduction } = this.props;
    const { initValue } = this.state;
    return (
      <div className="contact-modal-screen">
        <Modal isOpen={openSalaryComponentDeduction} className="modal-success contact-modal">
