...
import { Button, Input, Form, FormGroup, Row, Col, Table } from 'components/migration';
...
const mapStateToProps = state => {
  return {
    ...
    salary_component_deduction_dropdown:
      state.payrollEmployee.salary_component_deduction_dropdown.data,
  };
};
...
const SalaryComponent = ({
  ...
  const schema = z
    ...
    .refine(
      data => {
        if (data.CTC) {
          return (
            parseFloat(data.totalYearlyEarnings || 0) > parseFloat(data.totalYearlyDeductions || 0)
          );
        }
        return true;
      },
      {
        message:
          strings.TotalDeductionsShouldBeLessThanTotalEarnings ||
          'Total deductions should be less than total earnings',
  ...
  const getSalaryComponentByEmployeeId_First_Time = current_employee_id => {
    if (current_employee_id) {
      detailSalaryComponentAction
        .getSalaryComponentByEmployeeId(current_employee_id)
        .then(res => {
          if (res.status === 200) {
            const { ctcType } = formData;
            const ctc = res.data.ctc ?? 0;
            const monthltCTC =
              ctcType === 'Monthly' ? ctc : parseFloat(parseFloat(ctc / 12).toFixed(2));
  ...
  const removeComponent = ComponentId => {
    ...
    const deduction = Deduction
      ? Deduction.filter(obj => obj.salaryComponentId !== ComponentId)
      : '';
    const componentsSelected = componentSelected
      ? componentSelected.filter(obj => obj !== ComponentId)
      : [];
  ...
  const getSalaryComponentById = (componentId, componentType, index) => {
    ...
    createPayrollEmployeeActions
      .getSalaryComponentById(componentId)
      .then(res => {
        if (res.status === 200) {
          const data = componentType === 'Fixed' ? Fixed : Deduction;
  ...
  const renderComponentName = (row, index, componentType) => {
    const component_list =
      componentType === 'Fixed'
        ...
        : salary_component_deduction_dropdown;
    const description =
      component_list && component_list.length > 0
        ? component_list.find(obj => obj.value === row.salaryComponentId)
        : '';

    const unusedComponentList = [];
    component_list &&
      component_list.length > 0 &&
      component_list.forEach(obj => {
        if (!componentSelected.includes(obj.value)) unusedComponentList.push(obj);
      });
        ...
        onChange={e => {
          ...
          if (row.salaryComponentId) {
            componentUsed = componentUsed
              ? componentUsed.filter(obj => obj !== row.salaryComponentId)
              : [];
          }
              ...
              <Select
                ...
                value={
                  ...
                  selectOptionsFactory
                    .renderOptions('label', 'value', componentTypeOptions, 'Type')
                    .find(option =>
                      item.formula || item.formula === null ? option.value == 2 : option.value == 1
                    )
                }
                onChange={option => {
                  if (option.value == 1) {
                ...
                <tr key={index}>
                  ...
                  <td style={{ border: '3px solid #c8ced3' }}>
                    <Input
                      ...
                      value={
                        item.monthlyAmount
                          ? item.monthlyAmount.toLocaleString(navigator.language, {
                  ...
                  <td style={{ border: '3px solid  #c8ced3' }}>
                    {item.yearlyAmount
                      ? item.yearlyAmount.toLocaleString(navigator.language, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })
                      : '0.00'}
          ...
          <tr>
            <td colSpan={4} style={{ border: '3px solid  #c8ced3' }}>
              <Button
                ...
                className="pull-left"
                onClick={() => {
                  if (componentType === 'Fixed') setOpenSalaryComponentFixed(true);
          ...
          <tr style={{ background: '#dfe9f7', color: 'Black' }}>
            <td colSpan={2} style={{ border: '3px solid #c8ced3' }}>
              <b className="pull-left">
                {componentType === 'Fixed'
                  ? strings.TotalEarnings + ' (A):'
                  : strings.TotalDeductions + ' (B):'}
              </b>
            </td>
            <td style={{ border: '3px solid  #c8ced3' }}>
              <b>
                  ...
                  <FormGroup className="mt-3" style={{ textAlign: 'center', display: 'grid' }}>
                    <div style={{ display: 'flex', textAlign: 'center', justifyContent: 'center' }}>
                      ...
                      <div style={{ width: '20%', paddingRight: '2%' }}>
                        ...
                        />
                        {errors.CTC && <div className="invalid-feedback">{errors.CTC.message}</div>}
                      </div>
                      <div style={{ width: '20%' }}>
                        <Controller
                          name="ctcTypeOption"
                    ...
                    {errorMsg && errors.totalEarning && (
                      <div className="w-100 mr-5">
                        <div
                          ...
                          style={{ fontSize: 'medium', marginRight: '60px' }}
                        >
                    ...
                    <tbody>
                      <tr style={{ background: '#dfe9f7', color: 'Black' }}>
                        <td colSpan={2} style={{ border: '3px solid #c8ced3', width: '50%' }}>
                          <b className="pull-left">
                            {strings.Gross + ' ' + strings.Earnings + ' (C):'}
                          </b>
