...
const UpdateEmployeeEmployment = ({
    ...
    const [employeeChildActivitiesPresentOrNot, setEmployeeChildActivitiesPresentOrNot] = useState(false);
    ...
    useEffect(() => {
        if (location.state?.id) {
            detailEmployeeEmployementAction.getEmployeeById(location.state.id).then((res) => {
                if (res.status === 200) {
                    ...
                    setValue('dateOfJoining', res.data.dateOfJoining ? dayjs(res.data.dateOfJoining, 'DD-MM-YYYY').toDate() : null);
                    setValue('passportNumber', res.data.passportNumber || '');
                    setValue('passportExpiryDate', res.data.passportExpiryDate ? dayjs(res.data.passportExpiryDate, 'DD-MM-YYYY').toDate() : null);
    ...
    }, [location.state, detailEmployeeEmployementAction, createPayrollEmployeeActions, setValue, history]);

    const employeeValidationCheck = useCallback((value) => {
        const data = {
            moduleType: 15,
            name: value,
        };
    ...
    const laborCardIdValidationCheck = useCallback((value) => {
        const data = {
            moduleType: 23,
            name: value,
        };
    ...
    const onSubmit = (data) => {
        ...
        detailEmployeeEmployementAction.updateEmployment(formData).then((res) => {
            if (res.status === 200) {
            ...
            commonActions.tostifyAlert(
                'error',
                err.data?.message || 'Updated Unsuccessfully'
            );
            setLoading(false);
            setDisabled(false);
                                                            ...
                                                            <Col md="4">
                                                                <FormGroup>
                                                                    <Label htmlFor="employeeCode">
                                                                        ...
                                                                        <i id="employeeCodeTooltip" className="fa fa-question-circle ml-1"></i>
                                                                        <UncontrolledTooltip placement="right" target="employeeCodeTooltip">
                                                            ...
                                                            <Col md="4">
                                                                <FormGroup>
                                                                    <Label htmlFor="labourCard">
                                                                        ...
                                                                        <i id="labourCardTooltip" className="fa fa-question-circle ml-1"></i>
                                                                        <UncontrolledTooltip placement="right" target="labourCardTooltip">
                                                                    ...
                                                                    <Controller
                                                                        ...
                                                                        render={({ field }) => (
                                                                            <Input
                                                                                ...
                                                                                className={errors.labourCard || laborCardIdexist ? "is-invalid" : ""}
                                                                            />
                                                                        )}
                                                                    />
                                                                    {errors.labourCard && (
