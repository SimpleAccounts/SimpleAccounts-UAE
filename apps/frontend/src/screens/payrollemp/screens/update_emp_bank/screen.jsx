...
const UpdateEmployeeBank = ({
    ...
    useEffect(() => {
        createPayrollEmployeeActions.getBankListForEmployees()
            .then((response) => {
                setBankList(response.data);
            });

        if (location.state?.id) {
            detailEmployeeBankAction.getEmployeeById(location.state.id).then((res) => {
                if (res.status === 200) {
                    setLoading(false);
                    ...
                    setValue('bankId', res.data.bankId ? { value: res.data.bankId, label: res.data.bankName } : null);
    ...
    const checkAccountNumberExists = useCallback((value) => {
        const data = {
            moduleType: 19,
            name: value,
        };
    ...
    const onSubmit = (data) => {
        ...
        detailEmployeeBankAction.updateEmployeeBank(formData).then((res) => {
            if (res.status === 200) {
                                                                    ...
                                                                    <Controller
                                                                        ...
                                                                        render={({ field }) => (
                                                                            <Input
                                                                                ...
                                                                                className={errors.accountNumber || existForAccountNumber ? "is-invalid" : ""}
                                                                            />
                                                                        )}
                                                                    />
