...
const getBasicDetailsSchema = (sifEnabled, masterPhoneNumber) => {
  let schema = z.object({
    ...
    mobileNumber: z.string().min(1, 'Mobile number is required').refine((val) => val && val.length === 12, "Invalid mobile number"),
    dob: z.any().refine((val) => val, 'DOB is required').refine((val) => {
         // Age check > 14
         if (!val) return false;
    ...
    ...(sifEnabled ? {
        gender: z.object({ label: z.string(), value: z.string() }).refine(val => val.label !== 'Select Gender', 'Gender is required'),
        maritalStatus: z.object({ label: z.string(), value: z.string() }).refine(val => val.label !== 'Select Marital Status', 'Marital status is required'),
        presentAddress: z.string().min(1, 'Present address is required'),
        countryId: z.object({ label: z.string(), value: z.number() }).nullable().refine(val => val !== null, 'Country is required'),
        stateId: z.object({ label: z.string(), value: z.number() }).nullable().refine(val => val !== null, 'State/Emirate is required'),
        ...
        emergencyContactNumber1: z.string().min(1, 'Contact number 1 is required').refine((val) => val && val.length === 12, "Invalid mobile number").refine((val) => val !== masterPhoneNumber, "Please Enter Another Mobile Number"),
...
const getEmploymentSchema = () => z.object({
    employeeCode: z.string().min(1, 'Employee unique id is required'),
...
const getFinancialSchema = () => z.object({
    ...
    accountNumber: z.string().min(1, 'Account number is required').regex(/^[^0]+$/, 'Please enter a valid Account number'),
    bankId: z.object({ label: z.string(), value: z.number() }).refine(val => val, 'Bank is required'),
    branch: z.string().min(1, 'Branch is required'),
    iban: z.string().min(1, 'IBAN Number is required').regex(/^[^0]+$/, 'Please enter a valid IBAN Number'),
    swiftCode: z.string().optional(),
    agentId: z.string().min(9, 'Agent ID must be 9 characters').max(9, 'Agent ID must be 9 characters'),
});
...
const CreateEmployeePayroll = () => {
    ...
    const basicResolver = useCallback((values, context, options) => {
    ...
    const checkEmployeeCode = async (val) => {
        if (!val) return;
        const res = await dispatch(CreatePayrollEmployeeActions.checkValidation({ moduleType: 15, name: val }));
    ...
    const checkLabourCard = async (val) => {
        if (!val) return;
        const res = await dispatch(CreatePayrollEmployeeActions.checkValidation({ moduleType: 23, name: val }));
        setLabourCardExist(res.data === 'Labour Card Id Already Exists');
         if (res.data === 'Labour Card Id Already Exists') {
            employmentForm.setError('labourCard', { type: 'manual', message: 'Labour card id already exists' });
        } else {
    ...
    const checkAccountNumber = async (val) => {
        if (!val) return;
        const res = await dispatch(CreatePayrollEmployeeActions.checkValidation({ moduleType: 19, name: val }));
        setAccountNumberExist(res.data === 'Account Number Already Exists');
        if (res.data === 'Account Number Already Exists') {
             financialForm.setError('accountNumber', { type: 'manual', message: 'Account Number Already Exists' });
        } else {
    ...
    const onBasicSubmit = async (data) => {
        ...
        if (data.employeeDesignationId) formData.append('employeeDesignationId', data.employeeDesignationId.value);
        ...
        if (sifEnabled) {
            ...
            formData.append('qualificationYearOfCompletionDate', data.qualificationYearOfCompletionDate || '');
        ...
        try {
            ...
            if (res.status === 200) {
                toast.success(employeeId ? 'Employee Updated Successfully!' : 'Employee Basic Details Saved Successfully');
                ...
                if (!sifEnabled && !employeeId) {
                     ...
                     formData1.append('dateOfJoining', data.dateOfJoining ? dayjs(data.dateOfJoining).format('DD-MM-YYYY') : '');
    ...
    const onEmploymentSubmit = async (data) => {
        ...
        formData.append('dateOfJoining', data.dateOfJoining ? dayjs(data.dateOfJoining).format('DD-MM-YYYY') : '');
        formData.append('passportNumber', data.passportNumber || '');
        formData.append('passportExpiryDate', data.passportExpiryDate ? dayjs(data.passportExpiryDate).format('DD-MM-YYYY') : '');
        ...
        try {
            ...
            if (res.status === 200) {
                toast.success(selectedData.employmentId ? 'Employment Details Updated Successfully' : 'Employment Details Saved Successfully');
    ...
    return (
        <div className="financial-report-screen">
                    ...
                    <CardContent className="p-4">
                        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                            <TabsList className="grid w-full grid-cols-4">
                                ...
                                {sifEnabled && <TabsTrigger value="2" disabled={!employeeId}>{strings.Employment}</TabsTrigger>}
                                {sifEnabled && <TabsTrigger value="3" disabled={!employeeId}>{strings.FinancialDetails}</TabsTrigger>}
                                <TabsTrigger value="4" disabled={!employeeId}>{strings.SalarySetup}</TabsTrigger>
                            </TabsList>
                                            ...
                                            <div className="col-lg-10">
                                                        ...
                                                        <FormField
                                                            ...
                                                            render={({ field }) => (
                                                                <FormItem className="space-y-3">
                                                                    <FormLabel>{strings.Status} <span className="text-danger">*</span></FormLabel>
                                                                    <FormControl>
                                                                        <RadioGroup
                                                                        ...
                                                                        >
                                                                            ...
                                                                            <FormItem className="flex items-center space-x-3 space-y-0">
                                                                                ...
                                                                                <FormLabel className="font-normal">{strings.Inactive}</FormLabel>
                                                                            </FormItem>
                                                                        </RadioGroup>
                                                                    </FormControl>
                                                                </FormItem>
                                                ...
                                                <div className="row">
                                                    <div className="col-lg-4">
                                                        <FormField
                                                            ...
                                                            render={({ field }) => (
                                                                <FormItem>
                                                                    <FormLabel>{strings.FirstName} <span className="text-danger">*</span></FormLabel>
                                                                    <FormControl>
                                                                        <Input {...field} onChange={(e) => field.onChange(upperFirst(e.target.value))} />
                                                                    </FormControl>
                                                                    <FormMessage />
                                                                </FormItem>
                                                            )}
                                                    ...
                                                    <div className="col-lg-4">
                                                        <FormField
                                                            ...
                                                            render={({ field }) => (
                                                                <FormItem>
                                                                    <FormLabel>{strings.LastName} <span className="text-danger">*</span></FormLabel>
                                                                    <FormControl>
                                                                        <Input {...field} onChange={(e) => field.onChange(upperFirst(e.target.value))} />
                                                                    </FormControl>
                                                                    <FormMessage />
                                                ...
                                                <div className="row mt-3">
                                                    <div className="col-md-4">
                                                         <FormField
                                                            ...
                                                            render={({ field }) => (
                                                                <FormItem>
                                                                    <FormLabel>{strings.Email} <span className="text-danger">*</span></FormLabel>
                                                                    <FormControl>
                                                                        <Input {...field} onBlur={(e) => { field.onBlur(); checkEmail(e.target.value); }} />
                                                                    </FormControl>
                                                                    <FormMessage />
                                                                </FormItem>
                                                            )}
                                                    ...
                                                    <div className="col-md-4">
                                                        <FormField
                                                            ...
                                                            render={({ field }) => (
                                                                <FormItem>
                                                                    <FormLabel>{strings.DateOfBirth} <span className="text-danger">*</span></FormLabel>
                                                    ...
                                                    <div className="col-md-4">
                                                        <FormField
                                                            ...
                                                            render={({ field }) => (
                                                                <FormItem>
                                                                    <FormLabel>{strings.MobileNumber} <span className="text-danger">*</span></FormLabel>
                                                                    <PhoneInput
                                                                        country={'ae'}
                                                                        value={field.value}
                                                                        onChange={field.onChange}
                                                 ...
                                                 <div className="row mt-3">
                                                     <div className="col-md-8">
                                                          <FormField
                                                            ...
                                                            render={({ field }) => (
                                                                <FormItem>
                                                                    <FormLabel>{strings.Designation} <span className="text-danger">*</span></FormLabel>
                                                                    <ReactSelect
                                                                        options={designation_dropdown ? selectOptionsFactory.renderOptions('label', 'value', designation_dropdown, 'Employee Designation') : []}
                                                                        value={field.value}
                                                                        onChange={field.onChange}
                                                                    />
                                                                    <FormMessage />
                                                    ...
                                                    <div className="row mt-3">
                                                         <div className="col-md-4">
                                                            <FormField
                                                                ...
                                                                render={({ field }) => (
                                                                    <FormItem>
                                                                        <FormLabel>{strings.Gender} <span className="text-danger">*</span></FormLabel>
                                                                        <ReactSelect
                                                         ...
                                                         <div className="col-md-4">
                                                            <FormField
                                                                ...
                                                                render={({ field }) => (
                                                                    <FormItem>
                                                                        <FormLabel>{strings.maritalStatus} <span className="text-danger">*</span></FormLabel>
                                                                        <ReactSelect
                                                            ...
                                                            <FormField
                                                                ...
                                                                render={({ field }) => (
                                                                    <FormItem>
                                                                        ...
                                                                        <ReactSelect
                                                                             options={employee_list_dropdown.data ? selectOptionsFactory.renderOptions('label', 'value', employee_list_dropdown.data, 'Employee') : []}
                                                                             value={field.value}
                                                                             onChange={field.onChange}
                                                    ...
                                                    <div className="row">
                                                         <div className="col-md-8">
                                                            <FormField
                                                                ...
                                                                render={({ field }) => (
                                                                    <FormItem>
                                                                        <FormLabel>{strings.PresentAddress} <span className="text-danger">*</span></FormLabel>
                                                                        <Input {...field} />
                                                                        <FormMessage />
                                                                    </FormItem>
                                                                )}
                                                    ...
                                                    <div className="row mt-3">
                                                         <div className="col-md-4">
                                                            <FormField
                                                                ...
                                                                render={({ field }) => (
                                                                    <FormItem>
                                                                        <FormLabel>{strings.Country} <span className="text-danger">*</span></FormLabel>
                                                                        <ReactSelect
                                                                             options={country_list ? selectOptionsFactory.renderOptions('countryName', 'countryCode', country_list, 'Country') : []}
                                                                             value={field.value}
                                                                             onChange={(val) => {
                                                                                 field.onChange(val);
                                                                                 dispatch(CreatePayrollEmployeeActions.getStateList(val.value));
                                                         ...
                                                         <div className="col-md-4">
                                                            <FormField
                                                                ...
                                                                render={({ field }) => (
                                                                    <FormItem>
                                                                        <FormLabel>{strings.StateRegion} <span className="text-danger">*</span></FormLabel>
                                                                        <ReactSelect
                                                ...
                                                <div className="row mt-5">
                                                    <div className="col-lg-12 text-right">
                                                         <Button type="button" variant="outline" className="mr-2" onClick={() => navigate('/admin/master/employee')}>
                                                             {strings.Cancel}
                                                         </Button>
                                                         <Button type="submit">
                                         ...
                                         <div className="row">
                                              <div className="col-md-4">
                                                  <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel>{strings.employee_unique_id} <span className="text-danger">*</span></FormLabel>
                                                            <FormControl>
                                                                <Input {...field} onBlur={(e) => { field.onBlur(); checkEmployeeCode(e.target.value); }} />
                                                            </FormControl>
                                                            <FormMessage />
                                                        </FormItem>
                                                    )}
                                              ...
                                              <div className="col-md-4">
                                                  <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel>{strings.LabourCardId} <span className="text-danger">*</span></FormLabel>
                                                            <FormControl>
                                                                <Input {...field} onBlur={(e) => { field.onBlur(); checkLabourCard(e.target.value); }} />
                                                            </FormControl>
                                                            <FormMessage />
                                                        </FormItem>
                                                    )}
                                              ...
                                              <div className="col-md-4">
                                                   <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel>{strings.DateOfJoining} <span className="text-danger">*</span></FormLabel>
                                                            <div className="custom-datepicker-wrapper">
                                                                <DatePicker
                                                                    className="form-control"
                                                                    selected={field.value}
                                        ...
                                        <div className="row">
                                             <div className="col-md-4">
                                                  <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel>{strings.AccountHolderName} <span className="text-danger">*</span></FormLabel>
                                                            <FormControl>
                                                                <Input {...field} onChange={(e) => field.onChange(upperFirst(e.target.value))} />
                                                            </FormControl>
                                                            <FormMessage />
                                             ...
                                             <div className="col-md-4">
                                                  <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel>{strings.AccountNumber} <span className="text-danger">*</span></FormLabel>
                                                            <FormControl>
                                                                <Input {...field} onBlur={(e) => { field.onBlur(); checkAccountNumber(e.target.value); }} />
                                                            </FormControl>
                                                            <FormMessage />
                                                        </FormItem>
                                                    )}
                                             ...
                                             <div className="col-md-4">
                                                  <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel>{strings.BankName} <span className="text-danger">*</span></FormLabel>
                                                            <ReactSelect
                                        ...
                                        <div className="row mt-3">
                                              <div className="col-md-4">
                                                   <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel>{strings.Branch} <span className="text-danger">*</span></FormLabel>
                                                            <FormControl>
                                                                <Input {...field} />
                                                            </FormControl>
                                                            <FormMessage />
                                              ...
                                              <div className="col-md-4">
                                                   <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel>{strings.IBANNumber} <span className="text-danger">*</span></FormLabel>
                                                            <div className="flex">
                                        ...
                                        <div className="row mt-3">
                                             <div className="col-md-4">
                                                   <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel>{strings.agent_id} <span className="text-danger">*</span></FormLabel>
                                                            <FormControl>
             ...
             <DesignationModal
                ...
                createDesignation={(data) => dispatch(CreatePayrollEmployeeActions.createEmployeeDesignation(data))}
