...
describe('Organization Actions', () => {
  ...
  describe('createOrganization', () => {
    ...
    it('should handle API errors during organization creation', async () => {
      ...
      await expect(store.dispatch(actions.createOrganization(orgData))).rejects.toThrow(
        'Creation failed'
      );
    });
    ...
    it('should handle validation errors', async () => {
      ...
      await expect(store.dispatch(actions.createOrganization(invalidData))).rejects.toThrow(
        'Validation failed'
      );
    });
    ...
    it('should handle network timeout errors', async () => {
      ...
      await expect(store.dispatch(actions.createOrganization({ name: 'Test' }))).rejects.toThrow(
        'Network timeout'
      );
    });
  });
  ...
  describe('getIndustryTypeList', () => {
    ...
    it('should handle API errors when fetching industry types', async () => {
      ...
      await expect(store.dispatch(actions.getIndustryTypeList())).rejects.toThrow(
    ...
    it('should handle industry types with additional metadata', async () => {
      ...
      const dispatchedActions = store.getActions();
      expect(dispatchedActions[0].payload[0].description).toBe('Technology sector');
    });
    ...
    it('should handle network errors', async () => {
      ...
      await expect(store.dispatch(actions.getIndustryTypeList())).rejects.toThrow('Network Error');
    });
  });
  ...
  describe('Error handling across all actions', () => {
    ...
    it('should handle 403 forbidden errors', async () => {
      ...
      await expect(store.dispatch(actions.getIndustryTypeList())).rejects.toThrow('Forbidden');
    });
