...
const CreateReceipt = () => {
	...
	const onSubmit = (data) => {
		...
		const { receiptNo, receiptDate, contactId, amount, depositeTo, payMode, notes, referenceCode } = data;
												...
												<Controller
													...
													render={({ field }) => (
														<Select
															{...field}
															styles={selectStyles}
															options={tmpContact_list ? selectOptionsFactory.renderOptions('label', 'value', tmpContact_list, 'Customer Name') : []}
											...
											{data1.length > 0 ? (
												<div>
													<Row>
														<Col lg={4}>
															<FormGroup className="mb-3">
																<Label htmlFor="amount"><span className="text-danger">* </span>{strings.AmountReceived}</Label>
																<Controller
																	name="amount"
																	control={control}
																	render={({ field }) => (
																		<Input {...field} type="number" id="amount" placeholder={strings.Amount} className={errors.amount ? 'is-invalid' : ''} />
																	)}
																/>
															</FormGroup>
														</Col>
													...
													<Row>
														<Col lg={4}>
															<FormGroup className="mb-3">
																<Label htmlFor="receipt_date"><span className="text-danger">* </span>{strings.PaymentDate}</Label>
																<Controller
																	name="receiptDate"
																	control={control}
																	render={({ field }) => (
													...
													<Row>
														<Col lg={4}>
															<FormGroup className="mb-3">
																<Label htmlFor="payMode"><span className="text-danger">* </span> {strings.PaymentMode}</Label>
																<Controller
																	name="payMode"
																	control={control}
																	render={({ field }) => (
																		<Select
																			{...field}
																			options={pay_mode ? selectOptionsFactory.renderOptions('label', 'value', pay_mode, 'Mode') : []}
																			styles={selectStyles}
														...
														<Col lg={4}>
															<FormGroup className="mb-3">
																<Label htmlFor="depositeTo"><span className="text-danger">* </span> {strings.ReceivedThrough}</Label>
																<Controller
																	name="depositeTo"
																	control={control}
																	render={({ field }) => (
													...
													<Row className="mt-5">
														<Col lg={12} className="text-right">
															...
															<Button type="button" color="secondary" className="btn-square" onClick={() => navigate('/admin/income/receipt')}>
																{strings.Cancel}
															</Button>
														</Col>
													</Row>
