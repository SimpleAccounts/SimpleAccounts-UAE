...
const CreateCreditNoteModal = props => {
  ...
  useEffect(() => {
    if (
      props.selectedData !== selectedData ||
    ...
    props.selectedData,
    props.totalAmount,
    props.totalVatAmount,
    props.prefixData,
    props.invoiceNumber,
    props.id,
    props.totalExciseAmount,
  ]);
              ...
              <Select
                ...
                value={
                  ...
                  selectOptionsFactory
                    .renderOptions('name', 'id', props.product_list, 'Product')
                ...
                className={
                  ...
                  errors.invoiceLineItems[idx] &&
                  errors.invoiceLineItems[idx].productId
                    ? 'is-invalid'
                    : ''
                }
              />
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Select
            ...
            options={
              props.excise_list
                ? selectOptionsFactory.renderOptions('name', 'id', props.excise_list, 'Excise Tax')
                : []
            }
            value={
              props.excise_list &&
              selectOptionsFactory
                .renderOptions('name', 'id', props.excise_list, 'Excise Tax')
                .find(option => option.value === +row.original.exciseTaxId)
            }
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Select
            ...
            options={
              props.vat_list
                ? selectOptionsFactory.renderOptions('name', 'id', props.vat_list, 'VAT')
                : []
            }
            value={
              props.vat_list &&
              selectOptionsFactory
                .renderOptions('name', 'id', props.vat_list, 'VAT')
        ...
        cell: ({ row }) =>
          row.original.vatAmount === 0
            ? selectedData.currencyIsoCode +
              ' ' +
              row.original.vatAmount.toLocaleString(navigator.language, {
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.subTotal
            ? selectedData.currencyIsoCode +
              ' ' +
              row.original.subTotal.toLocaleString(navigator.language, { minimumFractionDigits: 2 })
            : '',
      },
    ];
  ...
  const onSubmit = formData => {
    ...
    props
      .createCreditNote(submitFormData)
      .then(res => {
        if (res.status === 200) {
          reset();
        ...
        <Form name="simpleForm" onSubmit={handleSubmit(onSubmit)} className="create-contact-screen">
          ...
          <ModalBody>
            ...
            <Row>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <DatePicker
                        ...
                        className={`form-control ${errors.creditNoteDate ? 'is-invalid' : ''}`}
                      />
                    )}
                  />
              ...
              <Col lg={3}>
                <FormGroup className="mb-3">
                  <Label htmlFor="remainingInvoiceAmount">{strings.RemainingInvoiceAmount}</Label>
                  <Input
                    type="text"
                  ...
                  {remainingAmountError && (
                    <div className="text-danger">{remainingAmountError}</div>
                  )}
                </FormGroup>
              </Col>
            </Row>
              ...
              <Col lg={12}>
                {errors.invoiceLineItems && typeof errors.invoiceLineItems === 'string' && (
                  <div className={errors.invoiceLineItems ? 'is-invalid' : ''}>
                    <div className="invalid-feedback">{errors.invoiceLineItems}</div>
                  </div>
                )}
                <DataTable
                  columns={columns}
                  ...
                  <div className="">
                    {selectedData.totalExciseAmount > 0 ? (
                      <div className="total-item p-2">
                        <Row>
                          <Col lg={6}>
                            <h5 className="mb-0 text-right">{strings.TotalExcise}</h5>
                          </Col>
                          <Col lg={6} className="text-right">
                            <label className="mb-0">
                              {selectedData.currencyIsoCode} &nbsp;
                              {totalExciseAmount > 0 &&
                                totalExciseAmount.toLocaleString(navigator.language, {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2,
                                })}
                            </label>
                    ...
                    {selectedData.discount != 0 && (
                      <div className="total-item p-2">
                        <Row>
                          <Col lg={6}>
                            <h5 className="mb-0 text-right">{strings.Discount}</h5>
                          </Col>
                          <Col lg={6} className="text-right">
                            <label className="mb-0">
                              {selectedData.currencyIsoCode} &nbsp;
                              {selectedData.discount != 0 &&
                                selectedData.discount?.toLocaleString(navigator.language, {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2,
                                })}
                            </label>
                    ...
                    <div className="total-item p-2">
                      <Row>
                        ...
                        <Col lg={6} className="text-right">
                          <label className="mb-0">
                            {selectedData.currencyIsoCode} &nbsp;
                            {total_net.toLocaleString(navigator.language, {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </label>
