...
class InvoiceTemplate extends Component {
      ...
      return (
        <div>
          ...
          <div className="mb-1 ml-2">
            <b>{invoiceData.organisationName ? invoiceData.organisationName : invoiceData.name}</b>
          </div>
  ...
  getTerms = term => {
    let val = this.termList && this.termList.find(option => option.value === term);
              ...
              >
                ...
                >
                  <div style={{ marginTop: '0.5rem' }}>
                    {companyData.isRegisteredVat ? (
                      <h1 className="mb-1 ml-2">{strings.TaxInvoice}</h1>
                    ) : (
                      <h1 className="mb-1 ml-2">{strings.CustomerInvoice}</h1>
                    )}
            ...
            >
              ...
              >
                <div>
                  ...
                  {invoiceData &&
                    ...
                    invoiceData.taxTreatment.includes('NON') == false && (
                      <div className="mb-1 ml-2">
                        {strings.VATRegistrationNo}: {invoiceData.taxRegistrationNo}
                      </div>
                    )}
                ...
                <div style={{ width: '27%' }}>
                  <br />
                  <div className="mb-1 ml-2">
                    <b>{strings.InvoiceNo}: </b> # {invoiceData.referenceNumber}
                  </div>
                  {invoiceData.receiptNumber && (
                    <div className="mb-1 ml-2">
                      <b>{strings.ReferenceNo}: </b>
                      {invoiceData.receiptNumber}
                    </div>
                  )}
                  <div className="mb-1 ml-2">
                    <b>{strings.InvoiceDate}: </b>{' '}
                    {dayjs(invoiceData.invoiceDate).format('DD-MM-YYYY')}
                  </div>
                  <div className="mb-1 ml-2">
                    <b>{strings.DueDate}: </b>
                    {dayjs(invoiceData.invoiceDueDate).format('DD-MM-YYYY')}
                  </div>
                  <div className="mb-1 ml-2">
                    <b>{strings.Terms}: </b>
                    {this.getTerms(invoiceData.term)}
                  </div>
                  <div className="mb-1 ml-2">
                    <b>{strings.Status}: </b>
                    {this.renderInvoiceStatus(invoiceData.status)}
                  </div>
                  <br />
                  <br />
                  <div className="mb-1 ml-2">
                    <strong style={{ padding: '0.5rem', background: '#f2f2f2' }}>
                      {strings.BalanceDue}:{' '}
                      {invoiceData.dueAmount
                        ? invoiceData.currencyIsoCode +
                          ' ' +
                          invoiceData.dueAmount.toLocaleString(navigator.language, {
                  ...
                  <div className="mb-1 ml-2 mt-1 w-100">
                    {invoiceData.exchangeRate && invoiceData.exchangeRate !== 1 && (
                      <strong style={{ padding: '0.5rem', background: '#f2f2f2' }}>
                        {strings.Exchangerate}: {invoiceData.exchangeRate}
                      </strong>
                    )}
                  </div>
                </div>
              ...
              <thead className="header-row" style={{ fontSize: '12px' }}>
                <tr>
                  ...
                  <th style={{ padding: '0.5rem', textAlign: 'right' }}>
                    {invoiceData.discount && invoiceData.discount > 0 ? strings.Discount : ''}
                  </th>
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <td style={{ textAlign: 'right', width: '10%' }}>
                          {invoiceData.currencyIsoCode +
                            ' ' +
                            item.unitPrice.toLocaleString(navigator.language, {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                        </td>
                            ...
                            ? item.discountType == 'PERCENTAGE'
                              ...
                              : currencyData[0]
                                ? currencyData[0].currencyIsoCode +
                                  ' ' +
                                  item.discount.toLocaleString(navigator.language, {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                  })
                                : 'AED' +
                        ...
                        <td style={{ textAlign: 'right' }}>
                          {item.exciseAmount && item.exciseAmount > 0 ? (
                            <>
                              {invoiceData.currencyIsoCode +
                                ' ' +
                                item?.exciseAmount.toLocaleString(navigator.language, {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2,
                                })}
                            </>
                        ...
                        <td style={{ textAlign: 'right' }}>
                          {item.vatAmount && item.vatAmount > 0
                            ? invoiceData.currencyIsoCode +
                              ' ' +
                              item.vatAmount.toLocaleString(navigator.language, {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              })
                            : null}
                        ...
                        <td style={{ textAlign: 'right' }}>
                          <b>
                            {invoiceData.currencyIsoCode +
                              ' ' +
                              item.subTotal.toLocaleString(navigator.language, {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              })}
                          </b>
                          ...
                          >
                            ...
                            <span>
                              {invoiceData.totalExciseAmount
                                ? invoiceData.currencyIsoCode +
                                  ' ' +
                                  invoiceData.totalExciseAmount.toLocaleString(navigator.language, {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                  })
                                : invoiceData.currencyIsoCode +
                          ...
                          >
                            ...
                            <span>
                              {invoiceData.discount
                                ? invoiceData.currencyIsoCode +
                                  ' ' +
                                  invoiceData.discount.toLocaleString(navigator.language, {
                                    minimumFractionDigits: 2,
                                  })
                                : invoiceData.currencyIsoCode +
                                  ' ' +
                            ...
                            {invoiceData.totalAmount
                              ? invoiceData.currencyIsoCode +
                                ...
                                (
                                  ...
                                  parseFloat(invoiceData.totalVatAmount) -
                                  parseFloat(invoiceData.totalExciseAmount)
                          ...
                          >
                            ...
                            <span>
                              {invoiceData.totalVatAmount
                                ? invoiceData.currencyIsoCode +
                                  ' ' +
                                  invoiceData.totalVatAmount.toLocaleString(navigator.language, {
                                    minimumFractionDigits: 2,
                                  })
                                : invoiceData.currencyIsoCode +
                                  ' ' +
                        ...
                        >
                          ...
                          <span>
                            {invoiceData.totalAmount
                              ? invoiceData.currencyIsoCode +
                                ' ' +
                                invoiceData.totalAmount.toLocaleString(navigator.language, {
                                  minimumFractionDigits: 2,
                                })
                              : 0}
                          </span>
