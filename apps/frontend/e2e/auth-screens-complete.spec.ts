...
test.describe('Authentication Screens - Complete Tests', () => {
	...
	test('should accept input in all text fields', async ({ page }) => {
		...
		await waitForFormFields(page, ['companyName', 'companyAddress1', 'firstName', 'lastName', 'email']);
		...
		if (controlVisible) {
			...
			if (menuVisible) {
				...
				if (optionCount > 0) {
					...
					const isOptionDisabled = await firstOption.getAttribute('aria-disabled').catch(() => null);
					...
					if (isOptionDisabled !== 'true') {
						...
						const errorFeedback = companyTypeSelect.locator('..').locator('..').locator('.invalid-feedback');
