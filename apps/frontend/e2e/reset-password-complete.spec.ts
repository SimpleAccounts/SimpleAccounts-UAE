...
test.describe('Reset Password Complete Flow', () => {
  ...
  test('should request password reset via forgotPassword endpoint', async ({ page }) => {
    ...
    const responsePromise = page
      .waitForResponse(response => response.url().includes('/public/forgotPassword'), {
        timeout: 30_000,
      })
      .catch(() => null);
    await submitButton.click();
  ...
  test('should reset password with valid token', async ({ page }) => {
    ...
    const hasConfirmPasswordError = await confirmPasswordError
    ...
    if (hasPasswordError || hasConfirmPasswordError) {
      ...
      const confirmPasswordErrorText = hasConfirmPasswordError
        ? await confirmPasswordError.textContent()
        : '';
    ...
    const navigationPromise = page
      .waitForURL(`**${LOGIN_PATH}**`, { timeout: 30_000 })
      .catch(() => null);
    ...
    if (response) {
      ...
      if (status !== 200) {
        ...
        console.log(
          'Reset password response body (first 200 chars):',
    ...
    if (apiError || !apiSuccess) {
      console.log(
        'API returned error or no success, verifying password change by attempting login...'
      );
      ...
      const oldPasswordResponsePromise = page
        .waitForResponse(response => response.url().includes('/auth/token'), { timeout: 30_000 })
        .catch(() => null);
      ...
      const newPasswordResponsePromise = page
        .waitForResponse(response => response.url().includes('/auth/token'), { timeout: 30_000 })
        .catch(() => null);
      await loginButton.click();
      ...
      if (newPasswordWorks && !oldPasswordWorks) {
        console.log(
          'Password was successfully changed (verified by successful login with new password and failure with old password)'
        );
    ...
    if (overallSuccess) {
      ...
      const hasLoginSuccess = await loginSuccessToast
  ...
  test('should handle invalid reset token', async ({ page }) => {
    ...
    await page.goto(`${BASE_URL}${RESET_PASSWORD_PATH}?token=invalid-token-12345`, {
      waitUntil: 'networkidle',
    });
    await page.waitForTimeout(3000);
